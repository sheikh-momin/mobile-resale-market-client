...
const AllReportedItems = () => {
    const { data: reportedItems = [], isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            try {
                const res = await fetch('https://mobile-resale-market-server-1.vercel.app/reportedItems', {
                    headers: {
    ...
    const handleDelete = id => {
        ...
        if (proceed) {
            fetch(`https://mobile-resale-market-server-1.vercel.app/reportedItems/${id}`, {
                method: 'DELETE',
                headers: {
                    authorization: `bearer ${localStorage.getItem('accessToken')}`
                },
