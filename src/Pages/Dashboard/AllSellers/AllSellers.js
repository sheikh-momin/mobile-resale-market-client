...
const AllSellers = () => {
    const { data: sellers = [], isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            try {
                const res = await fetch('https://mobile-resale-market-server-1.vercel.app/users/sellers', {
                    headers: {
    ...
    const handleMakeVerified = id => {
        fetch(`https://mobile-resale-market-server-1.vercel.app/users/seller/${id}`, {
            method: 'PUT',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            },
    ...
    const handleDelete = id => {
        ...
        if (proceed) {
            fetch(`https://mobile-resale-market-server-1.vercel.app/users/${id}`, {
                method: 'DELETE',
                headers: {
                    authorization: `bearer ${localStorage.getItem('accessToken')}`
                },
